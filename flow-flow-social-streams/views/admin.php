...
<div id="fade-overlay" class="loading">
	<div id="waiting-posts">
		...
		<p>Please wait until the fetching process is complete. Usually it's quick but it can take up to 2 min. If it
			takes longer or fails we recommend to ask for <a href="https://social-streams.com/contact/"
				target="_blank">our support</a> </p>
	</div>
	<div class="anim-loader-wrapper">
		<div class="anim-loader"></div>
	</div>

</div>

<div id="ff-popup-banner" class="">
	<div>Getting data for feed 'eagle' in background...<i class="flaticon-pinterest"></i></div><span
		class="banner-close">Close</span>
</div>
...
<form id="flow_flow_form" method="post" action="<?php echo $context['form-action']; ?>" enctype="multipart/form-data">
	<script id="flow_flow_script">
		...
		<?php if (isset($context['js-vars']))
			echo $context['js-vars']; ?>
	</script>
	<?php
	settings_fields('ff_opts');
	if (isset($context['hidden-inputs']))
		echo $context['hidden-inputs'];
	?>
	<div class="wrapper">
		<?php
		if (FF_USE_WP) {
			echo '<h2>' . $context['admin_page_title'] . (LAUtils::slug($context) == 'flow-flow-social-streams' ? ' v. ' : ' Feed Gallery v. ') . LAUtils::version($context) . ' <a href="' . $context['faq_url'] . '" target="_blank">Docs & FAQ</a></h2>';
		...
		<ul class="section-tabs">
			...
			if (isset($context['buttons-after-tabs']))
				echo $context['buttons-after-tabs'];
			?>
		</ul>
			...
			<div class="width-wrapper">
				<div class="ff-table">
					<div class="ff-cell">
						...
						<?php if (defined('FF_PLUGIN_VER'))
							echo 'Version: ' . FF_PLUGIN_VER; ?><br>
						Made by <a href="http://looks-awesome.com/">Looks Awesome</a>
					</div>
					<div class="ff-cell">
						<h1>HOT TOPICS</h1>
						<a href="http://flow-php.looks-awesome.com/docs/Getting_Started/First_Steps_After_Installation">How
							to add stream on page</a><br>
						<a
							href="http://flow-php.looks-awesome.com/docs/Getting_Started/First_Steps_After_Installation#refresh">How
							to refresh my streams</a><br>
						<a href="http://flow-php.looks-awesome.com/docs/Social_Networks_Auth/Authenticate_with_Facebook">How
							to authorize Facebook</a><br>
